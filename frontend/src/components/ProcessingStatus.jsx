...
const ProcessingStatus = ({ stage, isComplete }) => {
  ...
  const stages = [
    { num: 1, label: 'Uploading', icon: 'Upload' },
    { num: 2, label: 'Transcribing', icon: 'Mic' },
    { num: 3, label: 'Generating MOM', icon: 'Sparkles' },
  ...
  return (
    <div className="glass rounded-xl p-6">
      ...
      {isComplete && (
        <div className="mt-6 p-4 bg-gradient-to-r from-cyan-900/30 to-teal-900/30 border border-cyan-700/50 rounded-lg text-center">
          <p className="text-cyan-400 font-semibold">Processing Complete!</p>
        </div>
      )}
    </div>
  );
